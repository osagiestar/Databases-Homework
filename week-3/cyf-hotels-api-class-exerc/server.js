...
app.get("/customers/:customerId/bookings", function (req, res) {
  ...
  pool
    .query("SELECT bookings.checkin_date, bookings.nights, hotels.name, hotels.postcode FROM customers INNER JOIN bookings ON customers.id = bookings.customer_id INNER JOIN hotels ON hotels.id = bookings.hotel_id WHERE customers.id = $1", [customerId] )
    .then((result) => res.json(result.rows))
    .catch((e) => console.error(e));
});
